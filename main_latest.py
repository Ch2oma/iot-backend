...
def load_users():
    ...
    # Try to write this file back (may fail on read-only clouds)
    try:
...
# --- FIX: Restored original Netlify URL and kept Vercel just in case ---
origins = [
    "http://localhost:5173",
    "http://localhost:3000",
    "https://ee495smarthome.netlify.app",  # Restored from main.py
    "https://iot-backend-phi.vercel.app",
    "*" # Ideally avoid *, but useful for debugging if you change domains often
]
...
def on_connect(client, userdata, flags, rc):
    # Handles paho-mqtt v1 and v2 callback signatures
    if isinstance(rc, object) and hasattr(rc, "value"):
         rc = rc.value
...
# MQTT CLIENT SETUP
# =====================================================
# --- FIX: Added callback_api_version to match main.py and prevent errors ---
...
def send_command(cmd: Command):
    ...
    # Handle paho-mqtt v2 return type
    rc = r.rc if hasattr(r, "rc") else r
    print("Publishing:", message, "→ RC =", rc)
